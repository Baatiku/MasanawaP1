...
export function PublicHeader() {
  return <header className="sticky top-0 z-50 border-b border-white/7 bg-[#01130c]/88 backdrop-blur-xl">
    <div className="mx-auto flex h-[74px] max-w-7xl items-center justify-between px-5 md:px-8">
      <Link href="/" aria-label="Perfect Naira home" className="flex items-center gap-2.5"><img src="/perfect-naira-mark.svg" alt="" className="h-9 w-9"/><span className="text-lg font-extrabold tracking-[-.035em]">Perfect Naira</span></Link>
...
export function PublicFooter() {
  return <footer className="border-t border-white/7 bg-[#010f0a]">
    <div className="mx-auto max-w-7xl px-5 py-14 md:px-8">
      ...
      <div className="mt-12 flex flex-col gap-3 border-t border-white/7 pt-6 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between"><p>© {new Date().getFullYear()} Perfect Naira. All rights reserved.</p><Link href="/contact" className="inline-flex items-center gap-2 hover:text-emerald-400"><Mail size={14}/>Contact Perfect Naira</Link></div>
    </div>
  </footer>;
}

function FooterGroup({title,links}:{title:string;links:Array<[string,string]>}) { return <div><p className="text-sm font-bold">{title}</p><div className="mt-4 space-y-3">{links.map(([label,href])=><Link key={label} href={href} className="block text-xs text-slate-400 transition hover:text-emerald-400">{label}</Link>)}</div></div>; }

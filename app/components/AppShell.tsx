  ...
  return (
    <main className="min-h-screen pb-24 lg:pb-0">
      <aside className="glass fixed inset-y-0 left-0 z-30 hidden w-[248px] border-r border-white/8 px-5 py-6 lg:flex lg:flex-col">
        <Brand />
        <nav className="mt-10 space-y-2">{nav.map((item) => <Link key={item.label} href={item.href} className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-sm transition ${active === item.label ? "bg-cyan-400/12 text-cyan-300 ring-1 ring-cyan-300/15" : "text-slate-400 hover:bg-white/5 hover:text-white"}`}><item.icon size={19} strokeWidth={1.9} /><span>{item.label}</span></Link>)}</nav>
        <div className="mt-auto rounded-3xl border border-cyan-300/10 bg-cyan-300/[.055] p-4"><div className="mb-3 flex h-10 w-10 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><ShieldCheck size={20} /></div><p className="text-sm font-semibold">Secure your account</p><p className="muted mt-1 text-xs leading-5">Complete verification to unlock higher transaction limits.</p><Link href="/profile" className="mt-4 inline-block text-xs font-semibold text-cyan-300">Verify account →</Link></div>
      </aside>
      <section className="lg:ml-[248px]">
        <header className="glass sticky top-0 z-20 flex h-[76px] items-center justify-between border-b border-white/7 px-5 md:px-8 lg:px-10"><div className="lg:hidden"><Brand compact /></div><div className="hidden lg:block"><p className="text-sm font-semibold">{title}</p>{subtitle && <p className="muted mt-0.5 text-xs">{subtitle}</p>}</div><div className="flex items-center gap-3"><button className="relative flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 bg-white/[.035] text-slate-300"><Bell size={18} /></button><Link href="/profile" className="hidden items-center gap-3 sm:flex"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-300 to-sky-500 text-sm font-extrabold text-slate-950">{initials}</div><div className="hidden xl:block"><p className="text-xs font-semibold">{displayName}</p><p className="muted text-[11px]">{accountLabel || "Masanawa account"}</p></div></Link></div></header>

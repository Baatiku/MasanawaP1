...
const services = [
  ...
  { label: "Cable TV", icon: Tv, href: "/services/cable", tone: "text-emerald-400 bg-emerald-500/10" },
...
export default async function DashboardPage() {
  ...
  const fullName = profile?.full_name || "Perfect Naira customer";
    ...
    <AppShell active="Home" title="Dashboard" subtitle="Your money, services and digital assets." userName={fullName} accountLabel={`${humanize(profile?.kyc_status ?? "unverified")} account`}>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.32fr_.9fr]">
        <section className="balance-card min-h-[244px] p-5 sm:p-6 lg:p-7">
          <div className="relative z-10 flex h-full flex-col">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold text-emerald-50/75 sm:text-sm">Available balance</p>
              ...
              <span className="rounded-xl border border-emerald-300/30 bg-[#073b28]/65 px-3 py-2 text-[11px] font-bold text-emerald-50">{currency}</span>
            </div>
            <div className="mt-auto grid grid-cols-3 gap-2.5 border-t border-emerald-100/16 pt-5 sm:gap-3">
              <Link href="/wallet/fund" className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-2 text-[11px] font-extrabold text-[#021f14] shadow-[0_10px_30px_rgba(34,197,94,.16)] sm:text-xs"><ArrowDownToLine size={16}/>Fund</Link>
              <Link href="/wallet/transfer" className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-emerald-300/35 bg-[#052c1e]/45 px-2 text-[11px] font-bold text-white transition hover:bg-[#0a402b] sm:text-xs"><Send size={16}/>Transfer</Link>
              <Link href="/wallet/withdraw" className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-emerald-300/35 bg-[#052c1e]/45 px-2 text-[11px] font-bold text-white transition hover:bg-[#0a402b] sm:text-xs"><ArrowUpFromLine size={16}/>Withdraw</Link>
            </div>
          </div>
        </section>
        ...
        <section className="app-card flex min-h-[244px] flex-col p-5 sm:p-6">
          <div className="flex items-start justify-between">
            ...
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-300/10 text-amber-200"><Landmark size={19}/></span>
          </div>
          <div className="mt-5 flex items-center gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-[#d8b45b] to-[#9f7624] text-white shadow-lg shadow-[#0b2118]/40"><Landmark size={21}/></div>
          ...
          <div className="mt-auto grid grid-cols-2 gap-4 border-t border-[#214d38]/65 pt-4">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[.82fr_1.18fr]">
        <section className="app-card p-5 sm:p-6">
          <div className="flex items-center justify-between"><h2 className="text-sm font-bold">Quick services</h2><Link href="/services" className="text-[11px] font-bold text-emerald-400">View all</Link></div>
        ...
        <section className="app-card p-5 sm:p-6">
          <div className="flex items-center justify-between"><h2 className="text-sm font-bold">Crypto assets</h2><Link href="/crypto" className="flex items-center gap-1 text-[11px] font-bold text-emerald-400">View all <ChevronRight size={13}/></Link></div>
          <div className="mt-3 space-y-2">
            {markets.map(coin => (
              <Link href="/crypto" key={coin.symbol} className="flex items-center justify-between rounded-xl border border-[#235b41]/55 bg-[#07291d]/60 px-3 py-2.5 transition hover:border-emerald-400/25">
      ...
      <section className="app-card mt-4 overflow-hidden p-4 sm:p-5 lg:p-6">
        <div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">Recent transactions</h2><p className="muted mt-1 text-[10px]">Your latest account activity</p></div><Link href="/transactions" className="flex items-center gap-1 text-[11px] font-bold text-emerald-400">View all <ChevronRight size={13}/></Link></div>
        {transactions.length === 0 ? (
          ...
          <div className="mt-4">
            <div className="muted hidden grid-cols-[minmax(0,1.45fr)_.65fr_.55fr_.65fr] gap-4 border-b border-[#214d38]/65 px-2 pb-3 text-[9px] font-semibold uppercase tracking-[.08em] md:grid"><span>Transaction</span><span>Amount</span><span>Status</span><span className="text-right">Date & time</span></div>
            <div className="divide-y divide-[#214d38]/55">
              {transactions.map(tx => {
                ...
                return (
                  <Link href={`/transactions/${tx.id}`} key={tx.id} className="group grid grid-cols-[1fr_auto] items-center gap-3 px-1 py-3.5 transition hover:bg-white/[.018] md:grid-cols-[minmax(0,1.45fr)_.65fr_.55fr_.65fr] md:px-2">
                    <div className="flex min-w-0 items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${positive ? "bg-emerald-400/10 text-emerald-300" : "bg-amber-300/10 text-amber-200"}`}>{positive ? <ArrowDownToLine size={17}/> : <ArrowUpFromLine size={17}/>}</span><div className="min-w-0"><p className="truncate text-xs font-bold">{title}</p><p className="muted mt-1 truncate text-[9px]">{tx.reference}</p></div></div>

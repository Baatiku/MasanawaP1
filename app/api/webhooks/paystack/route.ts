...
async function handleFunding(admin: AdminClient, event: PaystackEvent, eventId: string) {
  ...
  if (!reference) {
    await finishWebhook(admin, eventId, "Successful charge has no reference");
    return true;
  }
  ...
  if (verified.status !== "success" || verified.reference !== reference || verified.currency !== "NGN" || !Number.isSafeInteger(amountMinor) || amountMinor <= 0) {
    await finishWebhook(admin, eventId, "Verified charge is invalid");
    return true;
  }
  ...
  if (intent) {
    if (amountMinor !== Number(intent.amount_minor) || intent.currency !== "NGN") {
  ...
  const dedicatedChannel = event.data?.authorization?.channel === "dedicated_nuban";
  if (!dedicatedChannel || !receiverAccount) {
    await finishWebhook(admin, eventId, "Charge does not match a funding intent or dedicated virtual account");
    return true;
  }

  const { data: virtualAccount } = await admin.from("virtual_accounts")
    .select("id,user_id,account_number,active")
    .eq("provider", "paystack").eq("account_number", receiverAccount).eq("active", true).maybeSingle();
  if (!virtualAccount) {
    await finishWebhook(admin, eventId, "Dedicated virtual account not found");
    return true;
  }
  ...
  const { error: creditError } = await admin.rpc("credit_wallet_deposit", {
    ...
    p_amount_minor: amountMinor,
    p_external_reference: reference,
    p_idempotency_key: `paystack:dva:${reference}`,
  });
  if (creditError) { await finishWebhook(admin, eventId, creditError.message); return true; }
  await finishWebhook(admin, eventId);
  return true;
...
async function handleDedicatedAccount(admin: AdminClient, event: PaystackEvent, eventId: string) {
  ...
  const { data: requestRow } = await admin.from("virtual_account_requests")
    .select("id,user_id,email,preferred_bank,status")
    .eq("email", email).eq("provider", "paystack").eq("status", "pending")
  ...
  let account: PaystackDedicatedAccount | null | undefined = data?.dedicated_account ?? {
    id: data?.id,
  ...
  if (!account?.account_number || !account.account_name || !account.bank?.name) {
    try {
      ...
      await finishWebhook(admin, eventId, error instanceof Error ? error.message : "Unable to fetch assigned virtual account");
      return true;
    }
  }
  if (!account?.account_number || !account.account_name || !account.bank?.name || account.currency !== "NGN") {
    await finishWebhook(admin, eventId, "Assigned virtual account response is incomplete");
    return true;
  }
...
export async function POST(request: Request) {
  ...
  const { error: eventInsertError } = await admin.from("webhook_events").insert({ provider_code: "paystack", event_id: eventId, event_type: event.event ?? null, signature_valid: true, payload: event });
  ...
  } else if (eventInsertError) return NextResponse.json({ ok: false }, { status: 500 });

...
export default async function FundWalletPage({ searchParams }: { searchParams?: Promise<{ error?: string; message?: string }> }) {
  ...
  return <div className="w-full"><div className="max-w-5xl">
    ...
    <section className="panel mt-7 rounded-[30px] p-5 md:p-7"><h2 className="text-sm font-bold">Fund with checkout</h2><p className="muted mt-1 text-xs">Create a payment request and complete it on the provider checkout.</p><form action={createFundingIntent} className="mt-5"><input type="hidden" name="idempotency_key" value={fundingIdempotencyKey}/><label htmlFor="amount" className="text-xs font-semibold">Amount (NGN)</label><input id="amount" name="amount" required type="number" min="100" step="100" placeholder="50000" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none placeholder:text-slate-600 focus:border-emerald-400/30"/><button type="submit" className="mt-4 flex w-full items-center justify-center rounded-2xl bg-emerald-400 py-3.5 text-sm font-bold text-slate-950">Continue to funding provider</button></form></section>

    <section className="panel mt-5 rounded-[30px] p-5 md:p-7"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold">Dedicated virtual account</p><p className="muted mt-1 text-xs">A personal account number for direct bank-transfer funding</p></div><Landmark className="text-emerald-400"/></div>
      {account ? <div className="soft-panel mt-5 rounded-2xl p-5"><p className="muted text-[11px] uppercase tracking-[.14em]">Account number</p><p className="mt-2 text-2xl font-extrabold tracking-wider">{account.account_number}</p><div className="mt-5 grid gap-4 border-t border-white/7 pt-4 sm:grid-cols-2"><div><p className="muted text-[11px]">Bank</p><p className="mt-1 text-sm font-semibold">{account.bank_name}</p></div><div><p className="muted text-[11px]">Account name</p><p className="mt-1 text-sm font-semibold">{account.account_name}</p></div></div><p className="muted mt-4 text-[11px] leading-5">Transfers into this account are credited only after a signed Paystack event is verified against this account number.</p></div> : profile?.kyc_status !== "verified" ? <div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.05] p-4 text-xs leading-5 text-amber-100">Complete <Link href="/profile/verification" className="font-bold text-emerald-400">identity verification</Link> before requesting a dedicated virtual account.</div> : lastRequest?.status === "pending" ? <div className="mt-5 rounded-2xl border border-emerald-400/15 bg-emerald-400/[.05] p-4 text-xs leading-5 text-slate-300">Your virtual-account request is being processed. We will notify you when Paystack finishes customer validation and account assignment.</div> : !providerReady || providerLoadError ? <div className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/[.05] p-4 text-xs leading-5 text-amber-100">{providerLoadError || "Virtual-account provisioning is not configured on this environment yet."}</div> : <form action={requestVirtualAccount} className="mt-5 space-y-4"><div className="grid gap-4 sm:grid-cols-2"><div><label className="text-xs font-semibold">Virtual-account bank</label><select name="preferred_bank" required className="mt-3 w-full rounded-2xl border border-white/8 bg-[#08291d] px-4 py-3 text-sm"><option value="">Select provider</option>{dvaProviders.map(item=><option key={item.provider_slug} value={item.provider_slug}>{item.bank_name}</option>)}</select></div><div><label className="text-xs font-semibold">Your validation bank</label><select name="bank_code" required className="mt-3 w-full rounded-2xl border border-white/8 bg-[#08291d] px-4 py-3 text-sm"><option value="">Select bank</option>{banks.map(bank=><option key={`${bank.code}-${bank.name}`} value={bank.code}>{bank.name}</option>)}</select></div></div><div><label className="text-xs font-semibold">Your bank account number</label><input name="bank_account_number" inputMode="numeric" pattern="[0-9]{10}" maxLength={10} required placeholder="10-digit account number" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none"/></div><div><label className="text-xs font-semibold">BVN</label><input name="bvn" type="password" inputMode="numeric" pattern="[0-9]{11}" maxLength={11} required autoComplete="off" placeholder="11-digit BVN" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm tracking-[.18em] outline-none"/><p className="muted mt-2 text-[11px] leading-5">Your BVN is sent directly from this server request to Paystack for identity validation and is not stored in Perfect Naira&apos;s database.</p></div><label className="flex items-start gap-3 rounded-2xl border border-white/7 bg-white/[.025] p-4 text-xs leading-5 text-slate-300"><input type="checkbox" name="consent" required className="mt-1 accent-emerald-400"/><span>I consent to Perfect Naira sending my legal name, phone number, bank-account details and BVN to Paystack for the purpose of validating me and assigning a dedicated virtual account.</span></label><button className="w-full rounded-2xl bg-emerald-400 py-3.5 text-sm font-bold text-slate-950">Request virtual account</button></form>}

...
export async function createBankWithdrawal(formData: FormData) {
  ...
  if (!isPaystackConfigured() || !process.env.SUPABASE_SECRET_KEY) {
    redirect(`/wallet/withdraw?error=${encodeURIComponent("Bank withdrawals are temporarily unavailable because the payout provider is not configured.")}`);
  }
  if (!Number.isFinite(amountNgn) || amountNgn < 100 || !bankCode || !/^\d{10}$/.test(accountNumber) || !/^\d{6}$/.test(pin)) {
    redirect(`/wallet/withdraw?error=${encodeURIComponent("Enter a valid bank, 10-digit account number, amount of at least ₦100 and six-digit PIN.")}`);
  }
  ...
  try {
    ...
    if (!bankName || !accountName || resolved.account_number !== accountNumber) {
    ...
    redirect(`/wallet/withdraw?error=${encodeURIComponent(message)}`);
  }
  ...
  if (!transactionId || !reference) {
    redirect(`/wallet/withdraw?error=${encodeURIComponent("Withdrawal request was created without a valid transaction reference.")}`);
  }
  ...
  try {
    ...
    if (saveBeneficiary) {
      await admin.from("beneficiaries").update({ recipient_code: recipient.recipient_code, updated_at: new Date().toISOString() })
        .eq("user_id", userId).eq("bank_code", bankCode).eq("account_number", accountNumber);
    }
    ...
    const transfer = await initiatePaystackTransfer({
      ...
      reason: `Masanawa withdrawal ${reference}`,
    });
    const providerStatus = String(transfer.status ?? "pending").toLowerCase();

    if (providerStatus === "success") {
      const { error: settleError } = await admin.rpc("settle_withdrawal_success", {
        p_transaction_id: transactionId,
        p_provider_reference: transfer.reference ?? providerReference,
      ...
      const { error: failError } = await admin.rpc("fail_withdrawal", {
        p_transaction_id: transactionId,
        p_failure_reason: `Paystack returned ${providerStatus}`,
        p_provider_status: providerStatus,
      });
      if (failError) throw failError;
    } else {
      const { error: markError } = await admin.rpc("mark_withdrawal_processing", {
        p_transaction_id: transactionId,
        p_recipient_code: recipient.recipient_code,
        p_transfer_code: transfer.transfer_code ?? "",
        p_provider_reference: transfer.reference ?? providerReference,
        p_provider_status: providerStatus,
      });
    ...
    const { error: releaseError } = await admin.rpc("fail_withdrawal", {
      p_transaction_id: transactionId,
      p_failure_reason: message,
      p_provider_status: "failed",
    });
    if (releaseError) {
      redirect(`/wallet/withdraw?error=${encodeURIComponent("The payout provider failed and the automatic release needs review. Please contact support with reference " + reference + ".")}`);
    }

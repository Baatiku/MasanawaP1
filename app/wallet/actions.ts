...
function ngn(formData: FormData, key = "amount") {
...
export async function createFundingIntent(formData: FormData) {
  ...
  if (reference && userData.user?.email && isPaystackConfigured() && process.env.SUPABASE_SECRET_KEY) {
    try {
      checkoutUrl = (await initializePaystackTransaction({ email: userData.user.email, amountMinor, reference })).authorization_url;
    } catch (error) {
      redirect(`/wallet/fund?error=${encodeURIComponent(error instanceof Error ? error.message : "Unable to initialize payment provider")}`);
    }
  }
...
export async function requestVirtualAccount(formData: FormData) {
  ...
  let successMessage = "Virtual-account assignment started";
  try {
    const result = await assignPaystackDedicatedAccount({
      email,
      firstName: parts[0],
  ...
  redirect(`/wallet/fund?message=${encodeURIComponent(`${successMessage}. We will notify you when the account is ready.`)}`);
}

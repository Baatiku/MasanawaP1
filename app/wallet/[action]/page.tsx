...
export default async function WalletActionPage({params,searchParams}:{params:Promise<{action:string}>;searchParams?:Promise<WalletActionSearchParams>}) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-2xl">
    ...
    {withdrawing ? <>
      {beneficiaries.length > 0 && <section className="mt-7"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-500">Saved beneficiaries</p><Link href="/wallet/withdraw" className="text-[11px] font-semibold text-cyan-300">Use another account</Link></div><div className="grid gap-3 sm:grid-cols-2">{beneficiaries.slice(0,4).map(item => <Link key={item.id} href={`/wallet/withdraw?beneficiary=${item.id}`} className={`rounded-2xl border p-4 transition ${selectedBeneficiary?.id===item.id?'border-cyan-300/30 bg-cyan-300/[.06]':'border-white/7 bg-white/[.025] hover:border-white/12'}`}><div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/[.045] text-cyan-300"><Landmark size={16}/></div><div className="min-w-0"><p className="truncate text-xs font-bold">{item.account_name}</p><p className="muted mt-1 text-[11px]">{item.bank_name}</p><p className="mt-1 font-mono text-[11px] text-slate-400">••••••{item.account_number.slice(-4)}</p></div></div></Link>)}</div></section>}
      ...
      {!payoutReady || providerError ? <section className="panel mt-7 rounded-[30px] p-7 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-300/10 text-amber-300"><ShieldCheck size={21}/></div><h2 className="mt-4 text-base font-bold">Bank withdrawals are temporarily unavailable</h2><p className="muted mx-auto mt-2 max-w-lg text-xs leading-6">{providerError || "Paystack payout credentials and the Supabase server secret must be configured before withdrawals can be submitted."}</p></section> : <form action={createBankWithdrawal} className="panel mt-7 rounded-[30px] p-5 md:p-7">
        <div><label htmlFor="bank_code" className="text-xs font-semibold">Bank</label><select id="bank_code" name="bank_code" required defaultValue={selectedBeneficiary?.bank_code ?? ""} className="mt-3 w-full rounded-2xl border border-white/8 bg-[#101a2b] px-4 py-3.5 text-sm outline-none focus:border-cyan-300/30"><option value="" disabled>Select bank</option>{banks.map(bank => <option key={`${bank.code}-${bank.name}`} value={bank.code}>{bank.name}</option>)}</select></div>
        <div className="mt-5"><label htmlFor="account_number" className="text-xs font-semibold">Account number</label><input id="account_number" name="account_number" required inputMode="numeric" pattern="[0-9]{10}" maxLength={10} defaultValue={selectedBeneficiary?.account_number ?? ""} placeholder="10-digit account number" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none placeholder:text-slate-600 focus:border-cyan-300/30"/><p className="muted mt-2 text-[11px]">The account name is resolved directly with Paystack before any wallet funds are reserved.</p></div>
        ...
        <label className="mt-5 flex items-center gap-3 rounded-2xl border border-white/7 bg-white/[.025] p-4 text-xs text-slate-300"><input type="checkbox" name="save_beneficiary" defaultChecked={Boolean(selectedBeneficiary)} className="accent-cyan-300"/><span>Save this verified bank account as a beneficiary.</span></label>
        <div className="mt-5 flex gap-3 rounded-2xl bg-cyan-300/[.045] p-4"><ShieldCheck className="mt-0.5 shrink-0 text-cyan-300" size={17}/><p className="muted text-xs leading-5">Funds are reserved in the ledger while Paystack processes the payout. A failed or reversed payout automatically releases the reserved amount back to your wallet.</p></div>
        <button type="submit" className="mt-6 flex w-full items-center justify-center rounded-2xl bg-cyan-300 py-3.5 text-sm font-bold text-slate-950">Verify account & withdraw</button>
      </form>}
    </> : <form action={transferToUsername} className="panel mt-7 rounded-[30px] p-5 md:p-7">

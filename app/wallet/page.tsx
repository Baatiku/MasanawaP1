...
import { ArrowDownToLine, ArrowUpFromLine, Clock3, Landmark, Send, WalletCards } from "lucide-react";
...
export default async function WalletPage() {
  ...
  const [profileResult, balanceResult, accountResult, transactionResult, holdResult] = await Promise.all([
  ...
  const heldMinor = Number(hold?.held_minor ?? 0);
  const heldCount = Number(hold?.held_count ?? 0);

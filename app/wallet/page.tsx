...
export default async function WalletPage() {
  ...
  const [profileResult, balanceResult, accountResult, transactionResult] = await Promise.all([
    ...
    supabase.from("transactions").select("kind,status,amount_minor,created_at").eq("user_id", userId).eq("status", "successful").order("created_at", { ascending: false }).limit(500),
  ]);
  ...
  const funded = rows.filter(tx => tx.kind === "deposit").reduce((sum, tx) => sum + Number(tx.amount_minor), 0);
  const spent = rows.filter(tx => !["deposit", "refund"].includes(tx.kind)).reduce((sum, tx) => sum + Number(tx.amount_minor), 0);
  const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0,0,0,0);
  const monthNet = rows.filter(tx => new Date(tx.created_at) >= monthStart).reduce((sum, tx) => sum + (["deposit", "refund"].includes(tx.kind) ? Number(tx.amount_minor) : -Number(tx.amount_minor)), 0);

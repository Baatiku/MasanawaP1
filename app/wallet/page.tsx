...
export default async function WalletPage() {
  ...
  return <AppShell active="Wallet" title="Wallet" subtitle="Fund, transfer and manage your naira balance." userName={fullName}>
    ...
    <div className="grid gap-4 xl:grid-cols-[1.32fr_.9fr]">
      <section className="balance-card min-h-[252px] p-5 sm:p-6 lg:p-7"><div className="relative z-10 flex h-full flex-col"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold text-cyan-50/75 sm:text-sm">Available balance</p><h2 className="tabular mt-3 text-[34px] font-extrabold leading-none tracking-[-.055em] sm:text-[43px] lg:text-[48px]">{money(Number(wallet?.balance_minor ?? 0), currency)}</h2>{heldMinor > 0 ? <p className="mt-3 flex items-center gap-1.5 text-[10px] font-semibold text-amber-200"><Clock3 size={13}/>{money(heldMinor, currency)} reserved across {heldCount} pending {heldCount === 1 ? "transaction" : "transactions"}</p> : <p className="mt-3 text-[10px] font-semibold text-emerald-200">Ledger-backed and available</p>}</div><span className="rounded-xl border border-cyan-200/30 bg-[#05243f]/65 px-3 py-2 text-[11px] font-bold text-cyan-50">{currency}</span></div><div className="mt-auto grid grid-cols-3 gap-2.5 border-t border-cyan-100/16 pt-5 sm:gap-3"><Link href="/wallet/fund" className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-cyan-300 px-2 text-[11px] font-extrabold text-[#032036] sm:text-xs"><ArrowDownToLine size={16}/>Fund</Link><Link href="/wallet/transfer" className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-cyan-200/35 bg-[#041c35]/45 px-2 text-[11px] font-bold sm:text-xs"><Send size={16}/>Transfer</Link><Link href="/wallet/withdraw" className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-cyan-200/35 bg-[#041c35]/45 px-2 text-[11px] font-bold sm:text-xs"><ArrowUpFromLine size={16}/>Withdraw</Link></div></div></section>
      <section className="app-card flex min-h-[252px] flex-col p-5 sm:p-6"><div className="flex items-start justify-between"><div><h2 className="text-sm font-bold">Virtual account</h2><p className="muted mt-1 text-[11px]">Use this account to fund Masanawa</p></div><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-400/12 text-indigo-300"><Landmark size={19}/></span></div><div className="mt-5 rounded-[18px] border border-[#29486f]/55 bg-[#081a34]/65 p-4"><p className="muted text-[9px] font-semibold uppercase tracking-[.1em]">Account number</p><p className="tabular mt-2 text-xl font-extrabold tracking-[.04em]">{account?.account_number ?? "Not assigned yet"}</p><div className="mt-4 border-t border-[#29415f]/60 pt-3"><p className="text-[11px] font-semibold">{account?.account_name ?? fullName}</p><p className="muted mt-1 text-[10px]">{account?.bank_name ?? "A funding account will appear here when provisioned"}</p></div></div><Link href="/wallet/beneficiaries" className="mt-auto flex items-center justify-between pt-4 text-[11px] font-bold text-slate-300"><span className="flex items-center gap-2"><UsersRound size={15} className="text-cyan-300"/>Bank beneficiaries</span><span className="flex items-center gap-1 text-cyan-300">{beneficiaryCountResult.count ?? 0}<ChevronRight size={13}/></span></Link></section>
    </div>
    <section className="app-card mt-4 p-5 sm:p-6"><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">Wallet activity</h2><p className="muted mt-1 text-[10px]">Successful ledger-backed transactions</p></div><Link href="/transactions" className="flex items-center gap-1 text-[11px] font-bold text-cyan-300">View history <ChevronRight size={13}/></Link></div><div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-4">{stats.map(item => <div key={item.label} className="rounded-[18px] border border-[#29486f]/55 bg-[#081a34]/60 p-4"><div className="flex items-center justify-between"><p className="muted text-[10px] font-semibold">{item.label}</p><WalletCards size={14} className="text-slate-600"/></div><p className={`tabular mt-3 truncate text-sm font-extrabold sm:text-base ${item.color}`}>{item.value}</p></div>)}</div></section>
  </AppShell>;
}

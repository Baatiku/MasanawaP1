...
export default async function TransactionsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  return <AppShell active="Transactions" title="Transactions" subtitle="Live activity from your Masanawa ledger." userName={profile?.full_name ?? "Masanawa user"}>
    <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between md:mb-6"><div><p className="text-sm font-medium text-slate-400">Account activity</p><h1 className="mt-0.5 text-2xl font-extrabold tracking-[-.04em] md:text-[28px]">Transactions</h1></div><Link href={exportHref} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#365274]/65 bg-[#08172e] px-4 text-[11px] font-bold text-slate-300 transition hover:border-cyan-300/25 hover:text-cyan-300"><Download size={15}/>Export CSV</Link></div>
    ...
    <form className="app-card grid gap-3 p-4 md:grid-cols-[1fr_.55fr_.65fr_auto_auto] md:p-5">
      <label className="relative"><span className="sr-only">Search transaction reference</span><Search size={15} className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500"/><input name="q" defaultValue={q} placeholder="Search reference" className="min-h-11 w-full rounded-xl border border-[#365274]/60 bg-[#06152b] py-3 pl-10 pr-3 text-[11px] outline-none placeholder:text-slate-600 focus:border-cyan-300/35"/></label>
      <label><span className="sr-only">Transaction status</span><select name="status" defaultValue={status} className="min-h-11 w-full rounded-xl border border-[#365274]/60 bg-[#06152b] px-3 text-[11px] outline-none focus:border-cyan-300/35"><option value="">All statuses</option>{statuses.filter(Boolean).map(item => <option key={item} value={item}>{humanize(item)}</option>)}</select></label>
      <label><span className="sr-only">Transaction type</span><select name="kind" defaultValue={kind} className="min-h-11 w-full rounded-xl border border-[#365274]/60 bg-[#06152b] px-3 text-[11px] outline-none focus:border-cyan-300/35"><option value="">All types</option>{kinds.filter(Boolean).map(item => <option key={item} value={item}>{humanize(item)}</option>)}</select></label>
      <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-cyan-300 px-4 text-[11px] font-extrabold text-[#032036]"><Filter size={14}/>Filter</button>
      {(q || status || kind) ? <Link href="/transactions" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#365274]/60 px-4 text-[11px] font-bold text-slate-400">Clear</Link> : null}
    </form>

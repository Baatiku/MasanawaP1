...
export default async function SupportPage({ searchParams }: { searchParams?: Promise<{ error?: string; message?: string }> }) {
  ...
  return <div className="w-full"><div className="max-w-5xl">
    ...
    <div className="mt-7"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-400/10 text-emerald-400"><LifeBuoy size={22}/></div><h1 className="mt-5 text-2xl font-bold md:text-3xl">Help & support</h1><p className="muted mt-2 text-sm">Open a support case, continue the conversation and track its status.</p></div>
    ...
    <section className="panel mt-7 rounded-[30px] p-5 md:p-7"><form action={createSupportCase} className="space-y-5"><div><label htmlFor="category" className="text-xs font-semibold">Category</label><select id="category" name="category" defaultValue="transaction" className="mt-3 w-full rounded-2xl border border-white/8 bg-[#08291d] px-4 py-3.5 text-sm outline-none"><option value="transaction">Transaction</option><option value="funding">Funding</option><option value="account">Account</option><option value="kyc">Verification</option><option value="crypto">Crypto</option><option value="other">Other</option></select></div><div><label htmlFor="subject" className="text-xs font-semibold">Subject</label><input id="subject" name="subject" required minLength={3} maxLength={120} className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none focus:border-emerald-400/30"/></div><div><label htmlFor="message" className="text-xs font-semibold">What happened?</label><textarea id="message" name="message" required minLength={10} maxLength={4000} rows={6} className="mt-3 w-full resize-none rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none focus:border-emerald-400/30"/></div><button type="submit" className="flex w-full items-center justify-center rounded-2xl bg-emerald-400 py-3.5 text-sm font-bold text-slate-950">Create support case</button></form></section>
    {(cases ?? []).length > 0 && <section className="panel mt-5 rounded-[30px] p-5 md:p-6"><h2 className="text-sm font-bold">Your cases</h2><div className="mt-3 divide-y divide-white/6">{(cases ?? []).map(item=><Link href={`/profile/support/${item.id}`} key={item.id} className="group flex items-center justify-between gap-4 py-4"><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.subject}</p><p className="muted mt-1 text-[11px]">{titleCase(item.category)} · Updated {new Date(item.updated_at).toLocaleString("en-NG")}</p></div><div className="flex items-center gap-3"><span className="shrink-0 text-xs text-emerald-400">{titleCase(item.status)}</span><ChevronRight size={15} className="text-slate-700 group-hover:text-emerald-400"/></div></Link>)}</div></section>}
  </div></div>;
}

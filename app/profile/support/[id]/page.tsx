...
export default async function SupportThreadPage({params,searchParams}:{params:Promise<{id:string}>;searchParams?:Promise<{error?:string;message?:string}>}) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-3xl">
    ...
    <div className="mt-7 flex items-start justify-between gap-4"><div><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><LifeBuoy size={22}/></div><h1 className="mt-5 text-2xl font-bold">{supportCase.subject}</h1><p className="muted mt-2 text-xs">{titleCase(supportCase.category)} · Opened {new Date(supportCase.created_at).toLocaleString("en-NG")}</p></div><span className="rounded-full border border-cyan-300/15 bg-cyan-300/[.06] px-3 py-1 text-[10px] font-semibold text-cyan-300">{titleCase(supportCase.status)}</span></div>
    ...
    <section className="panel mt-7 rounded-[30px] p-5 md:p-6"><div className="space-y-4">{(messages ?? []).map(message => <div key={message.id} className={`flex ${message.author_role==='user'?'justify-end':'justify-start'}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 ${message.author_role==='user'?'bg-cyan-300 text-slate-950':'border border-white/7 bg-white/[.035] text-slate-200'}`}><p className="text-sm leading-6 whitespace-pre-wrap">{message.message}</p><p className={`mt-2 text-[10px] ${message.author_role==='user'?'text-slate-800/70':'text-slate-500'}`}>{message.author_role==='user'?'You':'Masanawa Support'} · {new Date(message.created_at).toLocaleString("en-NG")}</p></div></div>)}</div></section>

    {supportCase.status !== 'closed' ? <form action={replySupportCase} className="panel mt-5 rounded-[30px] p-5 md:p-6"><input type="hidden" name="id" value={id}/><label htmlFor="message" className="text-xs font-semibold">Reply</label><textarea id="message" name="message" required maxLength={4000} rows={5} className="mt-3 w-full resize-none rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none focus:border-cyan-300/30"/><button className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-300 py-3.5 text-sm font-bold text-slate-950"><Send size={16}/>Send reply</button></form> : <div className="mt-5 rounded-2xl border border-white/8 bg-white/[.03] p-4 text-center text-xs text-slate-400">This case is closed. Open a new support case if you need more help.</div>}
  </div></main>;
}

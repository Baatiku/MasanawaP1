...
export default async function ReferralPage() {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-3xl">
    ...
    <div className="mt-7"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><Gift size={22}/></div><h1 className="mt-5 text-2xl font-bold md:text-3xl">Referrals & rewards</h1><p className="muted mt-2 text-sm">Invite people to Masanawa and track eligible rewards without exposing their private account details.</p></div>

    <section className="panel mt-7 rounded-[30px] p-5 md:p-7"><p className="text-xs font-semibold text-slate-400">Your referral code</p><p className="mt-2 text-2xl font-extrabold tracking-[.12em] text-cyan-300">{code || "Generating…"}</p><p className="muted mt-3 text-xs leading-5">Referral attribution is attached at account creation. Reward credits can only be posted by Masanawa&apos;s server-side reward process.</p><div className="mt-5"><CopyReferral value={referralLink}/></div></section>

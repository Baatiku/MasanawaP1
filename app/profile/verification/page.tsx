...
export default async function VerificationPage({ searchParams }: { searchParams?: Promise<{ error?: string; message?: string }> }) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-3xl">
    ...
    <section className="panel mt-5 rounded-[30px] p-5 md:p-6"><div className="flex items-center gap-3"><Gauge size={19} className="text-cyan-300"/><div><h2 className="text-sm font-bold">Your NGN transaction limits</h2><p className="muted mt-1 text-xs">Applied at the database boundary to outgoing transactions.</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="soft-panel rounded-2xl p-4"><p className="muted text-[10px] uppercase tracking-wide">Per transaction</p><p className="mt-2 text-lg font-bold">{money(perLimit)}</p></div><div className="soft-panel rounded-2xl p-4"><p className="muted text-[10px] uppercase tracking-wide">Rolling 24 hours</p><p className="mt-2 text-lg font-bold">{money(rollingLimit)}</p></div><div className="soft-panel rounded-2xl p-4"><p className="muted text-[10px] uppercase tracking-wide">Remaining now</p><p className="mt-2 text-lg font-bold text-cyan-300">{money(rollingRemaining)}</p></div></div><p className="muted mt-4 text-[11px] leading-5">Incoming transfers, deposits, refunds and administrative rewards do not consume this spending allowance. Pending and processing outgoing transactions are included so multiple requests cannot bypass the limit.</p></section>

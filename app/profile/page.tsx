...
const rows = [
  ...
  { label: "Wallet settings", text: "Accounts and beneficiaries", icon: WalletCards, href: "/wallet", tone: "bg-cyan-400/10 text-cyan-300" },
  ...
  { label: "Help & support", text: "Get assistance with Masanawa", icon: LifeBuoy, href: "/profile/support", tone: "bg-sky-400/10 text-sky-300" },
];
...
export default async function ProfilePage() {
  ...
  return <AppShell active="Profile" title="Profile" subtitle="Manage your Masanawa account and security." userName={fullName} accountLabel={`${titleCase(kyc)} account`}>
    ...
    <section className="app-card p-5 sm:p-6"><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-4"><div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border border-indigo-300/30 bg-gradient-to-br from-indigo-500 to-[#213968] text-lg font-extrabold text-white shadow-xl shadow-indigo-950/35">{initials}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="truncate text-lg font-extrabold tracking-[-.025em]">{fullName}</h2><span className={`rounded-lg border px-2 py-1 text-[8px] font-extrabold uppercase tracking-[.06em] ${kyc === "verified" ? "border-emerald-300/15 bg-emerald-300/[.07] text-emerald-300" : "border-amber-300/15 bg-amber-300/[.07] text-amber-300"}`}>{titleCase(kyc)}</span></div><p className="muted mt-1 truncate text-[11px]">{userData.user?.email || profile?.phone || "Masanawa account"}</p><p className="mt-2 text-[9px] font-semibold text-slate-500">Member since {memberSince}</p></div></div><form action={logout}><button type="submit" className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl border border-[#365274]/65 bg-[#08172e] px-4 text-[11px] font-bold text-slate-300 transition hover:border-rose-300/20 hover:text-rose-200 sm:w-auto"><LogOut size={15}/>Sign out</button></form></div></section>
    <section className="app-card mt-4 p-4 sm:p-5"><div className="grid gap-2.5 md:grid-cols-2">{rows.map(({ label, text, icon: Icon, href, tone }) => <Link href={href} key={label} className="group flex min-h-[76px] items-center justify-between gap-4 rounded-[16px] border border-[#29486f]/45 bg-[#081a34]/50 px-4 py-3 transition hover:border-cyan-300/25 hover:bg-[#0b2342]"><div className="flex min-w-0 items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${tone}`}><Icon size={18}/></span><div className="min-w-0"><p className="truncate text-xs font-bold">{label}</p><p className="muted mt-1 truncate text-[10px]">{text}</p></div></div><ChevronRight size={15} className="shrink-0 text-slate-600 transition group-hover:translate-x-0.5 group-hover:text-cyan-300"/></Link>)}</div></section>
  </AppShell>;
}

...
export default async function ServiceReviewPage({ searchParams }: { searchParams?: Promise<{ error?: string }> }) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-xl">
    ...
    <div className="mt-7 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><CheckCircle2 size={25}/></div><h1 className="mt-5 text-2xl font-bold md:text-3xl">Review transaction</h1><p className="muted mt-2 text-sm">Check every detail before authorizing this payment.</p></div>
    ...
    <section className="panel mt-7 rounded-[30px] p-5 md:p-7">
      ...
      <form action={confirmServiceOrder} className="mt-6">
        <div className="flex items-center justify-between"><label htmlFor="pin" className="text-xs font-semibold">Transaction PIN</label><Link href="/profile/security" className="text-[11px] font-semibold text-cyan-300">Set or change PIN</Link></div>
        <input id="pin" name="pin" required type="password" inputMode="numeric" autoComplete="off" pattern="[0-9]{6}" maxLength={6} placeholder="••••••" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-center text-lg tracking-[.6em] outline-none placeholder:text-slate-600 focus:border-cyan-300/30"/>
        <div className="mt-5 flex items-start gap-3 rounded-2xl bg-cyan-300/[.045] p-4"><ShieldCheck className="mt-0.5 shrink-0 text-cyan-300" size={17}/><p className="muted text-xs leading-5">The displayed product and price are loaded from the active catalog. PostgreSQL revalidates the product, amount, wallet balance and PIN atomically before reserving funds.</p></div>
        <button className="mt-6 flex w-full items-center justify-center gap-2 rounded-2xl bg-cyan-300 py-3.5 text-sm font-bold text-slate-950"><LockKeyhole size={17}/>Confirm and pay</button>
      </form>
    </section>
  </div></main>;
}

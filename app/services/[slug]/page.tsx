...
export default async function ServiceDetailPage({params,searchParams}:{params:Promise<{slug:string}>;searchParams?:Promise<ServiceSearchParams>}) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:py-10"><div className="mx-auto max-w-3xl">
    ...
    {(products ?? []).length === 0 ? <section className="panel mt-7 rounded-[30px] p-7 text-center"><div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-300/10 text-amber-300"><ShieldCheck size={21}/></div><h2 className="mt-4 text-base font-bold">No verified products are active yet</h2><p className="muted mx-auto mt-2 max-w-lg text-xs leading-6">Perfect Naira only exposes a service after its price and provider route have been verified by an administrator. Nothing is routed through placeholder providers.</p></section> : <div className="mt-7 space-y-4">{(products ?? []).map(product => {
      const fixed = product.pricing_mode === "fixed";
      return <form action={prepareServiceOrder} key={product.id} className="panel rounded-[30px] p-5 md:p-7">
        ...
        <div className="flex items-start justify-between gap-4"><div><h2 className="text-sm font-bold">{product.display_name}</h2><p className="muted mt-1 text-[11px]">{product.network || item.title}</p></div><span className="rounded-xl border border-emerald-400/15 bg-emerald-400/[.06] px-3 py-2 text-xs font-semibold text-emerald-400">{fixed ? money(Number(product.amount_minor ?? 0)) : "Flexible amount"}</span></div>
        <div className="mt-5 grid gap-4 sm:grid-cols-2"><div><label htmlFor={`recipient-${product.id}`} className="text-xs font-semibold">{item.label}</label><input id={`recipient-${product.id}`} name="recipient" required placeholder={item.placeholder} className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none placeholder:text-slate-600 focus:border-emerald-400/30"/></div>{!fixed && <div><label htmlFor={`amount-${product.id}`} className="text-xs font-semibold">Amount (NGN)</label><input id={`amount-${product.id}`} name="amount" required type="number" min={Number(product.min_amount_minor ?? 100)/100} max={product.max_amount_minor ? Number(product.max_amount_minor)/100 : undefined} step="1" placeholder="Enter amount" className="mt-3 w-full rounded-2xl border border-white/8 bg-white/[.035] px-4 py-3.5 text-sm outline-none placeholder:text-slate-600 focus:border-emerald-400/30"/></div>}</div>
        <div className="mt-5 flex items-start gap-3 rounded-2xl bg-emerald-400/[.045] p-4"><ShieldCheck className="mt-0.5 shrink-0 text-emerald-400" size={17}/><p className="muted text-xs leading-5">You will review the active catalog price, recipient and total before entering your transaction PIN.</p></div>

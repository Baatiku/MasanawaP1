...
export default async function ReadinessPage() {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-5xl">
    ...
    <div className="mt-7 flex items-start gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300"><ServerCog size={22}/></div><div><h1 className="text-2xl font-bold md:text-3xl">Deployment readiness</h1><p className="muted mt-2 text-sm">Configuration presence only. Secret values are never displayed.</p></div></div>
    ...
    <section className="panel mt-5 rounded-[30px] p-5 md:p-6"><h2 className="font-bold">Provider activation gate</h2><p className="muted mt-2 text-xs leading-5">Masanawa remains fail-closed until at least one provider, product and provider route are active. Activating records without real credentials does not make the system ready.</p><div className="mt-4 grid grid-cols-3 gap-3 text-center"><div className="soft-panel rounded-2xl p-4"><p className="text-xl font-extrabold">{activeProviders ?? 0}</p><p className="muted mt-1 text-[10px]">Active providers</p></div><div className="soft-panel rounded-2xl p-4"><p className="text-xl font-extrabold">{activeProducts ?? 0}</p><p className="muted mt-1 text-[10px]">Active products</p></div><div className="soft-panel rounded-2xl p-4"><p className="text-xl font-extrabold">{activeRoutes ?? 0}</p><p className="muted mt-1 text-[10px]">Active routes</p></div></div></section>
  </div></main>;
}

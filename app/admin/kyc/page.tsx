...
export default async function AdminKycPage({searchParams}:{searchParams?:Promise<{error?:string;message?:string}>}) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-6xl">
    <div className="flex items-center gap-4"><Link href="/admin" className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 bg-white/[.035]"><ArrowLeft size={18}/></Link><div><div className="flex items-center gap-2"><ShieldCheck size={18} className="text-cyan-300"/><h1 className="text-2xl font-extrabold tracking-[-.035em]">KYC review</h1></div><p className="muted mt-1 text-xs">Review private identity evidence and approve or reject verification cases.</p></div></div>

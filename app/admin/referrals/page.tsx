...
export default async function AdminReferralsPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-6xl">
    <div className="flex items-center gap-4"><Link href="/admin" className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 bg-white/[.035]"><ArrowLeft size={18}/></Link><div><div className="flex items-center gap-2"><Gift size={18} className="text-cyan-300"/><h1 className="text-2xl font-extrabold tracking-[-.035em]">Referrals & rewards</h1></div><p className="muted mt-1 text-xs">Review attributed signups and credit rewards through the server-only ledger function.</p></div></div>
    ...
    <div className="mt-5 space-y-3">{(referrals ?? []).length === 0 ? <section className="panel rounded-[30px] p-12 text-center text-sm text-slate-400">No referrals match this filter.</section> : (referrals ?? []).map(row => {
      ...
      return <section key={row.id} className="panel rounded-[26px] p-5"><div className="grid gap-5 lg:grid-cols-[1.25fr_1fr_.7fr_1.1fr]">
        ...
        <div><p className="muted text-[10px] uppercase tracking-wide">State</p><span className={`mt-2 inline-flex rounded-full border px-2.5 py-1 text-[10px] font-semibold ${row.status === "rewarded" ? "border-emerald-300/20 bg-emerald-300/[.07] text-emerald-300" : row.status === "rejected" ? "border-rose-300/20 bg-rose-300/[.07] text-rose-300" : "border-amber-300/20 bg-amber-300/[.07] text-amber-300"}`}>{titleCase(row.status)}</span>{Number(row.reward_minor) > 0 ? <p className="mt-3 text-sm font-bold text-cyan-300">{money(Number(row.reward_minor))}</p> : null}</div>
        <div>{canReward ? <form action={grantReferralReward} className="rounded-2xl border border-white/7 bg-white/[.025] p-3"><input type="hidden" name="referred_user_id" value={row.referred_user_id}/><label className="text-[10px] font-semibold uppercase text-slate-500">Reward NGN</label><input name="amount" required type="number" min="1" max="1000000" step="0.01" placeholder="500.00" className="mt-2 w-full rounded-xl border border-white/8 bg-white/[.03] px-3 py-2 text-xs outline-none"/><input name="reason" maxLength={500} placeholder="Qualification reason" className="mt-2 w-full rounded-xl border border-white/8 bg-white/[.03] px-3 py-2 text-[11px] outline-none"/><button className="mt-2 w-full rounded-xl bg-cyan-300 py-2.5 text-[11px] font-bold text-slate-950">Credit reward</button></form> : <div className="rounded-2xl border border-white/7 bg-white/[.025] p-4 text-xs text-slate-400">{row.status === "rewarded" ? `Rewarded ${row.rewarded_at ? new Date(row.rewarded_at).toLocaleString("en-NG") : ""}` : "Referral rejected"}</div>}</div>
      </div></section>;
    })}</div>
  </div></main>;
}
...
function Stat({ icon: Icon, label, value }: { icon: typeof Gift; label: string; value: string }) {
  return <div className="panel rounded-[24px] p-4"><div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-cyan-300/9 text-cyan-300"><Icon size={16}/></div><p className="mt-3 text-lg font-extrabold">{value}</p><p className="muted mt-1 text-[11px]">{label}</p></div>;
}

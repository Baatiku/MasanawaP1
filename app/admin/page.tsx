...
export default async function AdminPage({ searchParams }: { searchParams?: Promise<{ error?: string; message?: string }> }) {
  ...
  const [{ data: providers }, { data: products }] = await Promise.all([
    supabase.from("providers").select("code,name,category,active,priority,updated_at").order("category").order("priority"),
  ...
  return <main className="min-h-screen px-5 py-6 md:px-8 lg:px-12 lg:py-10"><div className="mx-auto max-w-7xl">
    <div className="flex items-center justify-between gap-4"><div className="flex items-center gap-4"><Link href="/" className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/8 bg-white/[.035]"><ArrowLeft size={18}/></Link><div><div className="flex items-center gap-2"><ShieldCheck size={18} className="text-cyan-300"/><h1 className="text-2xl font-extrabold tracking-[-.035em]">Masanawa Admin</h1></div><p className="muted mt-1 text-xs">Provider routing and service catalog controls.</p></div></div></div>
    ...
    <section className="panel mt-7 rounded-[30px] p-5 md:p-6"><div className="flex items-center gap-3"><ServerCog size={20} className="text-cyan-300"/><div><h2 className="font-bold">Providers</h2><p className="muted mt-1 text-xs">Inactive providers cannot receive new traffic.</p></div></div><div className="mt-5 grid gap-4 lg:grid-cols-2">{(providers ?? []).map(provider => <form action={updateProvider} key={provider.code} className="soft-panel rounded-2xl p-4"><input type="hidden" name="code" value={provider.code}/><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-bold">{provider.name}</p><p className="muted mt-1 text-[11px]">{provider.code} · {provider.category}</p></div><label className="flex items-center gap-2 text-xs"><input type="checkbox" name="active" defaultChecked={provider.active} className="accent-cyan-300"/> Active</label></div><div className="mt-4 flex items-end gap-3"><div className="flex-1"><label className="text-[11px] font-semibold">Priority</label><input name="priority" type="number" min="1" max="1000" defaultValue={provider.priority} className="mt-2 w-full rounded-xl border border-white/8 bg-white/[.035] px-3 py-2.5 text-sm outline-none"/></div><button className="rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-bold text-slate-950">Save</button></div></form>)}</div></section>
